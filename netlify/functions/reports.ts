  ...
  try {
    ...
    if (event.httpMethod === "POST") {
      try {
        ...
        if (!parsed.success) return errorResponse("Invalid report payload.", 400, parsed.error.flatten(), ENDPOINT);
        ...
        return errorResponse("Report save failed.", 500, error instanceof Error ? error.message : error, ENDPOINT);
      }
    }

    return errorResponse("Method not allowed.", 405, null, ENDPOINT);
  } catch (error) {
    return errorResponse("Reports function failed.", 500, error instanceof Error ? error.message : error, ENDPOINT);
  }
};
