...
import { fallbackReport, generateAiReports } from "./ai-report";
...
export async function runFastMarketScan(): Promise<ScanResponse> {
  const warnings: string[] = [];
  const scored: ScoredSetupInput[] = [];
  const newsByTicker: Record<string, Awaited<ReturnType<typeof fetchNews>>> = {};
  ...
  return {
    generatedAt: new Date().toISOString(),
    disclaimer:
      "Research only, not financial advice. Fast scan uses fetched quotes/news and deterministic reports; no order placement.",
    regime,
    setups: eligible.map((setup) => ({
      ...setup,
      regime,
      ai: fallbackReport(setup, regime)
    })),
    skipped,
    warnings
  };
